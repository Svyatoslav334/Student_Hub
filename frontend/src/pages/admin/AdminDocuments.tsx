    ...
    <div>
      <div className="flex justify-between items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm sm:text-base">Управління файлами, шаблонами і формами</p>
        </div>
        <div className="flex gap-2 shrink-0">
          <button
            onClick={() => setOpenCategoryModal(true)}
            className="px-3 py-2.5 sm:px-4 sm:py-2 bg-slate-800 rounded-xl text-sm"
          >
            <span className="hidden sm:inline">Категорії</span>
            <span className="sm:hidden">Кат.</span>
          </button>
          <button
            onClick={() => { setEditDoc(null); setOpenModal(true); }}
            className="px-3 py-2.5 sm:px-4 sm:py-2 bg-cyan-500 text-black rounded-xl flex items-center gap-2 text-sm"
          >
            <Plus size={18} />
          ...
          {documents.map((doc) => (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-sm text-slate-400">{doc.category?.name} • {doc.type}</p>
              </div>
              <div className="flex gap-2 items-center shrink-0">
                {doc.file && (
                  <a href={doc.file} target="_blank" className="p-2 bg-cyan-500/10 text-cyan-400 rounded-xl">
                    <File size={18} />
                  </a>
                )}
                <button
                  onClick={() => { setEditDoc(doc); setOpenModal(true); }}
